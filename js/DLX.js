...
function initBitMatrix() {
  var bitMatrix = make2d(rows+1);
  // Building Block matricies
  var I = [[1, 0, 0], [0, 1, 0], [0, 0, 1]];
  var l = [[1], [1], [1]];

  // Building up the constraints using kronecker products
  rc = kron(kron(kron(I, I), kron(I, I)), kron(l, l));
  rn = kron(kron(kron(I, I), kron(l, l)), kron(I, I));
  cn = kron(kron(l, l), kron(kron(I, I), kron(I, I)));
  bn = kron(kron(kron(kron(I, l), I), l), kron(I, I));

  // Add row of 0's at top :/
  bitMatrix[0] = new Array(cols).fill(0);

  // Combine the four arrays side by side
  for (var i = 0; i < rows; i++)
    bitMatrix[i+1] = rc[i].concat(rn[i].concat(cn[i].concat(bn[i])));
  return bitMatrix
}
  ...
  for (var i = 0; i <= rows; i++) {
    for (var j = 0; j < cols; j++) {
      ...
      if (bitMatrix[i][j] || !i) {
        ...
        a = i; b = j;
        do {b = getLeft(b);} while(!bitMatrix[a][b] && i && b != j);
        nodeMatrix[i][j].left = nodeMatrix[i][b];

        a = i; b = j;
        do {b = getRight(b);} while(!bitMatrix[a][b] && i && b != j);
        nodeMatrix[i][j].right = nodeMatrix[i][b];

        a = i; b = j;
        do {a = getUp(a);} while(!bitMatrix[a][b] && a && a != i);
        nodeMatrix[i][j].up = nodeMatrix[a][j];

        a = i; b = j;
        do {a = getDown(a);} while(!bitMatrix[a][b] && a && a != i);
...
function cover(target) {
  ...
  for (var rowNode = colNode.down; rowNode.rowID != colNode.rowID; rowNode = rowNode.down) {
    for (var rightNode = rowNode.right; rightNode.colID != rowNode.colID; rightNode = rightNode.right) {
...
function uncover(target) {
  ...
  // Move down and remove nodes traversing to the right
  for (var rowNode = colNode.up; rowNode.rowID != colNode.rowID; rowNode = rowNode.up) {
    for (var leftNode = rowNode.left; leftNode.colID != rowNode.colID; leftNode = leftNode.left) {
...
function getNodeFromRow(row) {
  for (var i = 0; i < cols; i++) {
    if (nodeMatrix[row][i] && nodeMatrix[row][i].column.left.right == nodeMatrix[row][i].column)
...
function search() {
  ...
  for (var rowNode = column.down; rowNode.rowID != column.rowID; rowNode = rowNode.down) {
